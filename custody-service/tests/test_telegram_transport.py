...
def provider_response(*, chat_id: int = -100123, message_id: int = 77, date: int = 1_788_000_000) -> bytes:
    return json.dumps(
        {
            "ok": True,
            "result": {
...
class TelegramBotApiTransportTests(unittest.TestCase):
    ...
    def test_out_of_scope_chat_is_rejected_before_network(self) -> None:
        transport = self.transport()
        message = TelegramOutboundMessage(-100999, "do not send")
        with patch(
    ...
    def test_send_message_uses_official_https_endpoint_and_returns_minimized_claim(self) -> None:
        transport = self.transport()
        message = TelegramOutboundMessage(-100123, "governed hello")
        captured = {}
        ...
        self.assertEqual(
            {"chat_id": -100123, "text": "governed hello"},
    ...
    def test_provider_denial_and_destination_mismatch_fail_closed(self) -> None:
        ...
        with patch(
            "pulpo_custody_service.telegram_transport.urllib_request.urlopen",
            return_value=FakeResponse(provider_response(chat_id=-100999)),
        ):
