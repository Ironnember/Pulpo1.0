...
class AuthorityApiTests(unittest.TestCase):
    ...
    def test_http_surface_exposes_authenticated_request_poll_and_human_ceremony(self):
        ...
        self.assertIn("Request received", display.text)
        self.assertIn("Review consequence", display.text)
        self.assertIn("Verify and approve", display.text)
        self.assertIn("Exact consequential object", display.text)
    ...
    def test_human_page_invokes_only_the_existing_webauthn_assertion_route(self):
        ...
        self.assertIn("approved hardware authenticator", page.text)
        self.assertIn("The interface cannot grant authority", page.text)
