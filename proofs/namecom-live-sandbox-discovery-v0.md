...
Advance the authenticated Name.com ceremony to the first real provider-backed step without creating a provider-side consequence. This proof authenticates two separately retained sandbox credential surfaces, performs provider-native domain availability/pricing discovery, and freezes one disposable non-premium candidate below Pulpo's $30 purchase ceiling.

This proof does **not** register a domain.
...
- executor and observer tokens must both authenticate and must be distinct;
- discovery endpoint: `POST /core/v1/domains:checkAvailability`;
- purchase type: `registration` only;
- maximum candidate batch: 30 domains, below the provider's documented 50-domain limit;
...
The runner reads the username and tokens interactively, tokens with terminal echo disabled, exports them only to the child process, then unsets them on exit.
...
1. `GET /core/v1/hello` using the executor credential;
2. `GET /core/v1/hello` using the observer credential;
3. `POST /core/v1/domains:checkAvailability` using the observer credential.
...
- purchase ceiling;
- booleans proving both credential surfaces authenticated and were distinct;
...
Until that provider-native attribution path is implemented and independently reviewed, authenticated read-only discovery may proceed but the domain-registration FIRE remains blocked.

## Claim boundary

If this proof succeeds:

- `Verified`: two distinct sandbox token surfaces authenticated against the real Name.com sandbox endpoint;
...
- `Not proven`: independent authority acceptance, provider write containment, exact provider-order attribution, registration success, reconciliation, or external consequence governance.
